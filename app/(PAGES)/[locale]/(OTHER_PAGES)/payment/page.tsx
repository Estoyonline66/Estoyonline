...
export default function PaymentPage() {
  ...
  useEffect(() => {
    if (status === "success") setStatusMessage(t("Ödemeniz alınmıştır. Teşekkürler", "Your payment has been received, thank you"));
    else if (status === "cancel") setStatusMessage(null);
  }, [status]);
  ...
  const handleContinue = async () => {
    ...
    for (let i = 0; i < requiredStudents; i++) {
      ...
      if (parts.length < 2) {
        setError(t(`Öğrenci ${i + 1}: Lütfen ad ve soyad giriniz.`, `Student ${i + 1}: Please enter first and last name.`));
        return;
      }
      const firstName = parts[0];
      if (firstName.length < 2 || !nameRegex.test(firstName)) {
        setError(t(`Öğrenci ${i + 1}: Ad en az 2 harf olmalı ve sayı içermemeli.`, `Student ${i + 1}: First name must be at least 2 letters and contain no numbers.`));
        return;
      }
      const lastName = parts.slice(1).join(" ");
      for (const word of lastName.split(" ")) {
        if (word.length < 3 || !nameRegex.test(word)) {
          setError(t(`Öğrenci ${i + 1}: Soyad en az 3 harf olmalı ve sayı içermemeli.`, `Student ${i + 1}: Last name must be at least 3 letters and contain no numbers.`));
          return;
        }
      }
    }
  ...
  if (!courseParam) return <p>{t("Geçersiz kurs linki", "Invalid course link")}</p>;
